...
// Notes..: cl65 -v [-O] -t atari appdemo.c -o appdemo.xex
// ------------------------------------------------------------
...
// ------------------------------------------------------------
// Func...: byte ListInput(void)
// Desc...: Demo use of GList gadget
// Returns: TRUE if accepted, else FALSE
// ------------------------------------------------------------
byte ListInput(void)
{
    byte bW1, bS;
    unsigned char *paA[21] = { "One      ", "Two      ", "Three    ", "Four     ", "Five     ",
                               "Six      ", "Seven    ", "Eight    ", "Nine     ", "Ten      ",
                               "Eleven   ", "Twelve   ", "Thirteen ", "Fourteen ", "Fifteen  ",
                               "Sixteen  ", "Seventeen", "Eighteen ", "Nineteen ", "Twenty   " };

    // Open window & draw form
    bW1 = WOpen(28, 8, 11, 8, WOFF, WON);
    WOrn(bW1, WPTOP, WPCNT, "List");

    // Display control in display mode
    bS = GList(bW1, 1, 1, GDISP, 12, 6, 20, paA);
    GAlert("Display Mode at 12");

    // Clear window
    WClr(bW1);

    // Display control in edit mode
    GAlert("Edit Mode at 3");
    bS = GList(bW1, 1, 1, GEDIT, 3, 6, 20, paA);

    // Show item selected
    if ((bS != XESC) && (bS != XTAB)) {
        GAlert(paA[bS-1]);
    } else {
        GAlert("Nothing selected.");
    }
...
void About(void)
{
    ...
    // Show window
    bW1 = WOpen(1, 6, 38, 15, WOFF, WON);
    WOrn(bW1, WPTOP, WPLFT, "About");
    WPrint(bW1, WPCNT,  1, WOFF, "Demo Application");
    WPrint(bW1, WPCNT,  2, WOFF, "Version 1.10-C");
    WPrint(bW1, WPCNT,  3, WOFF, "(C) 2022-2024  Wade Ripkowski");
    ...
    WPrint(bW1, 7,      9, WOFF, "V1-2021-Atari8: Action!");
    WPrint(bW1, 5,     10, WOFF, "V1-C-2022-Atari8: C (CC65)");
    WPrint(bW1, 2,     11, WOFF, "V1.45-C-2024-Atari8: C (CC65)");
...
void SubMenu(void)
{
    ...
    while (! bD) {
        ...
        bC = Menu(bW1, 1, 1, GVERT, WOFF, bC, 3, pcM);
...
void main(void)
{
    ...
    byte bD = FALSE;
    unsigned char *pcM[7] =
      { " Input Form   ", " List Input   ", " Progress Bar ", " Sub-Menu     ", " About        ", " Exit         " };
    ...
    WPrint(bW1, WPCNT, 1, WON, "D E M O N S T R A T I O N");

    // Display status window
    GStat(WON, "GStat window: 2s delay...");
    sleep(2);
    GStat(WOFF, "");

    // Open menu window
    bW2 = WOpen(12, 7, 16, 10, WOFF, WON);
    WOrn(bW2, WPTOP, WPCNT, "Menu");

    // Loop until done (Exit selected)
    while (! bD) {
        // Call menu
        bC = Menu(bW2, 1, 2, GVERT, WON, bC, 6, pcM);
        ...
        switch (bC) {
            ...
            case 6: if (GConf("Exiting the demo application...") == GCYES) {
                        bD = TRUE;
                    }
                    break;
        }
